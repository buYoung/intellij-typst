...
`src/main/grammar`는 Typst 문법 생성 입력(`Typst.flex`, `Typst.bnf`)을 두는 위치다. 이 입력으로 생성된 렉서/파서/PSI 코드는 손으로 작성한 소스와 섞이지 않도록 `build/generated/sources/grammarkit-lexer`와 `build/generated/sources/grammarkit-parser` 아래로 출력되며, 버전 관리 대상이 아니다. 생성 워크플로와 출력 경로 계약은 [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md)의 "Typst 렉서/파서 소스 생성" 절을 따른다.
...
- 생성되는 렉서는 `com.livteam.typninja.language.lexer`, 생성되는 파서/PSI는 `com.livteam.typninja.language.psi`(필요 시 `language.parser`) 패키지를 기준으로 한다. 생성된 코드는 `build/generated/sources/grammarkit-*` 아래에 출력되고 손으로 작성한 같은 패키지의 코드와 디렉터리가 겹치지 않는다.
